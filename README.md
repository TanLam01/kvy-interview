...
- **Webhook Callback**: Simulates async processing delay (5–15 seconds), randomly dispatching `verified` (45%), `rejected` (10%), or `inconclusive` (45%) results via webhook POST.
...
5.  Start the NestJS dev server:
    ...
    npm run start:dev
    ```
    _The backend runs on `http://localhost:3000/api`._
